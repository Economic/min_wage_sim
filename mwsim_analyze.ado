...
qui forvalues i = 1 / `steps' {
    ...
    use direct`i' indirect`i' d_annual_inc`i' direct`i' indirect`i' d_wage`i' d_annual_inc`i' perwt`i' using `microdata', clear
    gen byte pop = 1
    gcollapse ///
...
qui foreach x in direct indirect {
    ...
    forvalues i = 1 / `steps' {
        noi di as txt "... step `i'"
        use d_wage`i' d_annual_inc`i' d_wage`i' d_annual_inc`i' perwt`i' `x'`i' using `microdata', clear
        gcollapse ///
            (sum) d_annual_inc_`x' = d_annual_inc`i' ///
